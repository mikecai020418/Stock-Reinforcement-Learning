...
class TestEnv(gym.Env):
    ...
    def _take_action(self, action_prob_val):
        ...
        if action_type < 1:
            ...
            additional_cost = shares_bought * current_price*1.02  #Trading cost
            ...
            self.balance += shares_sold * current_price *0.98  #Trading cost
    ...
    def step(self, action_prob_val):
        ...
        delay_modifier = 1 #(self.current_step / MAX_STEPS)
...
#         action_type = float(action_prob_val[0])
#         probs_list= action_prob_val[3]
#         amount= tf.sigmoid(sorted(probs_list, reverse=True)[0] - sorted(probs_list, reverse=True)[1])
#         print(amount)
#         #amount=0.1
#         #amount = float(action_prob_val[1])  #by certainty 
#         #amount = (1-action_prob_val[1]/(-4))  #This is the percentage of stocks that shall be traded. -10 is an attempt to normalize the result to be [0,1], considering p<0.0001
...
#         current_date = self.df.loc[self.current_step, "Date"]  #For recording
#         current_close_price = self.df.loc[self.current_step, "Close"]
...
#             shares_bought = int(total_possible * amount)
#             prev_cost = self.cost_basis * self.shares_held
#             additional_cost = shares_bought * current_price * 1.02 
#             #print("ttl",total_possible,"shares b",shares_bought)
#             self.balance -= additional_cost
#             self.cost_basis = (prev_cost + additional_cost) / (self.shares_held + shares_bought+1e-6)
#             self.shares_held += shares_bought
...
#             # Sell amount % of shares held
#             shares_sold = int(self.shares_held * amount)
#             self.balance += shares_sold * current_price * 0.98
#             self.shares_held -= shares_sold
#             self.total_shares_sold += shares_sold
#             self.total_sales_value += shares_sold * current_price
#             self.transactions.append((current_date, current_close_price, 'sell', shares_sold)) #viz
